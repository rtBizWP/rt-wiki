 ...
 * rtWiki
 *
 * The RtWikiAttributes Class. To add attributes page in admin panel
 *
 * @package    RtWikiAdmin
 * @subpackage Admin
 *
...
if ( ! class_exists( 'RtWikiAttributes' ) ){
	...
	class RtWikiAttributes
	{
		...
		function attributes_page()
		{
			...
			if ( ! empty( $_GET[ 'edit' ] ) ) $this->rtwiki_edit_attribute(); else
		...
		function perform_action()
		{
			...
			if ( 'add' === $action || 'edit' === $action ){
				...
				if ( ! $attribute_name || ! $attribute_name ){
					...
					$taxonomy_exists = $rtWikiAttributesModel->attribute_exists( rtwiki_sanitize_taxonomy_name( $attribute_name ), $_GET[ 'p_type' ] );
				...
				if ( ! empty( $error ) ){
					...
					if ( 'add' === $action ){
						...
						$attribute = array( 'attribute_label' => $attribute_label, 'attribute_name' => $attribute_name, 'attribute_orderby' => $attribute_orderby, 'attribute_post_type' => $attribute_post_type );
					...
					if ( 'edit' === $action ){
						...
						$attribute = array( 'attribute_label' => $attribute_label, 'attribute_orderby' => $attribute_orderby );
						...
						if ( $old_attribute_name != $attribute_name && ! empty( $old_attribute_name ) ){
							...
							$wpdb->update( $wpdb->term_taxonomy, array( 'taxonomy' => rtwiki_attribute_taxonomy_name( $attribute_name ) ), array( 'taxonomy' => $old_attribute_name ) );
						}
						$action_completed = true;
					}
				}
			...
			<div class="wrap">
				...
				<form
						...
						<tr class="form-field form-required">
							...
							<td>
								<select name="attribute_orderby" id="attribute_orderby">
									<option
										value="menu_order" <?php selected( $att_orderby, 'menu_order' ); ?>><?php _e( 'Custom ordering' ); ?></option>
									<option
					...
					<input type='hidden' name='p_type'
						   value='<?php echo esc_html( isset( $_GET[ 'p_type' ] ) ? $_GET[ 'p_type' ] : ( ( isset( $_GET[ 'post_type' ] ) ? $_GET[ 'post_type' ] : '' ) ) ); ?>'/>
		...
		function rtwiki_add_attribute()
		{
			...
			if ( $attribute_taxonomies ) :
				...
				?>
									<tr>
									<td colspan="6"><?php _e( 'No attributes currently exist.' ) ?></td></tr>
				<?php
			endif;
			?>
								</tbody>
						...
						<div class="col-wrap">
							<div class="form-wrap">
								...
								<form
									...
									<input type='hidden' name='p_type' value='<?php echo esc_html( $post_type ); ?>'/>
